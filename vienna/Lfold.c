            ...
            int maxdist)
{
  ...
  ll = malloc( sizeof(struct mnode) );
  struct mnode* toReturn = ll;
  ll->next = 0;
        ...
        if (zsc){
          ...
          if (prev) {
            if ((i+strlen(ss)<prev_i+strlen(prev)) || strncmp(ss+prev_i-i,prev,strlen(prev))){
              ...
              if (dangles==2) {
                //printf("max .%s (%6.2f) %4d\n", prev, (f3[prev_i]-f3[prev_i+strlen(prev)-1])/100., prev_i-1);
                ll->fe = (f3[prev_i]-f3[prev_i+strlen(prev)-1])/100.;
                ll->sstruct = malloc(strlen(prev)+2);
                ll->sstruct[0] = '.';
                ll->left = prev_i-1;
                strcpy(ll->sstruct+1, prev);
              } else {
                //printf("max %s (%6.2f) %4d\n", prev, (f3[prev_i]-f3[prev_i+strlen(prev)])/100., prev_i);
                ll->fe = (f3[prev_i]-f3[prev_i+strlen(prev)])/100.;
                ll->sstruct = malloc(strlen(prev)+1);
                ll->left = prev_i;
                strcpy(ll->sstruct, prev);
              }
              ll->next = malloc(sizeof(struct mnode));
              ll = ll->next;
              ll->next = 0;
            }
            free(prev);
        ...
        if (do_backtrack) {
          ...
          if(zsc){
            ...
            if (dangles==2) {
              ...
              ll->fe = (f3[lind]-f3[lind+strlen(ss)-1])/100.;
            } else {
              //printf("max %s (%6.2f) %4d\n", ss, (f3[lind]-f3[lind+strlen(ss)])/100., 1);
              ll->fe = (f3[lind]-f3[lind+strlen(ss)])/100.;
            }
            ll->left = 1;
            ll->sstruct = malloc(strlen(ss)+1);
            strcpy(ll->sstruct, ss);
            ll->next = malloc(sizeof(struct mnode));
            ll = ll->next;
            ll->next = 0;
            free(ss);
